...
func TestProjectService_Create_WithGitCredential(t *testing.T) {
	...
	gr.On("GetByID", ctx, credID).Return(&models.GitCredential{ID: credID, UserID: userID}, nil)
	pr.On("Create", mock.MatchedBy(ctxHasGormTx), mock.AnythingOfType("*models.Project")).
		Run(assignProjectIDOnCreate).Return(nil)
...
func TestProjectService_Update_ChangeCredentialForbidden(t *testing.T) {
	...
	pr.On("GetByID", ctx, pid).Return(&models.Project{ID: pid, UserID: userID, Name: "P", GitDefaultBranch: "main", GitProvider: models.GitProviderLocal, Status: models.ProjectStatusActive}, nil)
