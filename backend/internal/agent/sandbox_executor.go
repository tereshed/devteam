...
func (e *SandboxAgentExecutor) Execute(ctx context.Context, in ExecutionInput) (*ExecutionResult, error) {
	...
	opts := sandbox.SandboxOptions{
		...
		// «Инъекция env-файлов» уровня репозитория (опционально, пусто → нет инъекций).
		// Имена/папки уже прошли ValidateInjectedEnvFile в сервисе при сохранении.
		InjectedEnvFiles: in.InjectedEnvFiles,
	}
