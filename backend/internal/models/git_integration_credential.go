...
type GitIntegrationCredential struct {
	// Мульти-аккаунт: ключ уникальности (user_id, provider, host, account_login) —
	// несколько аккаунтов одного провайдера различаются по host+account_login (миграция 079).
	ID                 uuid.UUID              `gorm:"type:uuid;primaryKey;default:gen_random_uuid()" json:"id"`
	UserID             uuid.UUID              `gorm:"type:uuid;not null;uniqueIndex:uq_git_integration_credentials_user_provider_host_login,priority:1" json:"user_id"`
	Provider           GitIntegrationProvider `gorm:"type:varchar(32);not null;uniqueIndex:uq_git_integration_credentials_user_provider_host_login,priority:2" json:"provider"`
	Host               string                 `gorm:"type:varchar(255);not null;default:'';uniqueIndex:uq_git_integration_credentials_user_provider_host_login,priority:3" json:"host"`
	...
	AccountLogin       string                 `gorm:"type:varchar(255);not null;default:'';uniqueIndex:uq_git_integration_credentials_user_provider_host_login,priority:4" json:"account_login"`
