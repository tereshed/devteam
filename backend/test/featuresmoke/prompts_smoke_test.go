...
// promptsAuthCases — общий набор ручек для AssertRequiresAuth/AssertRequiresAdmin.
// Каждая новая admin-only ручка /api/v1/prompts добавляется одной строкой —
// и сразу попадает под оба контракта (401 без токена, 403 без админа).
...
func TestPrompts_RequireAdminForNonAdminUser(t *testing.T) {
	...
	h := StartServer(t)
	user := h.NewUser(t)
	h.AssertRequiresAdmin(t, user.AccessToken, promptsAuthCases())
}
