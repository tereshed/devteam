...
/// Диалог OAuth device-flow для Antigravity.
///
/// Поведение:
///   1. На open вызывает `POST /antigravity/auth/init` и получает `user_code` + URL.
///   2. Кнопка «Open browser» → `url_launcher` уводит юзера на Antigravity.
...
Future<void> showConnectAntigravityDialog(
  ...
  await showDialog<void>(
    ...
    builder: (ctx) => _ConnectAntigravityDialog(parentRef: ref),
  );
}

enum _OAuthDialogPhase { initializing, awaiting, success, error, timeout }

class _ConnectAntigravityDialog extends ConsumerStatefulWidget {
  const _ConnectAntigravityDialog({required this.parentRef});

  final WidgetRef parentRef;

  @override
  ConsumerState<_ConnectAntigravityDialog> createState() =>
      _ConnectAntigravityDialogState();
}

class _ConnectAntigravityDialogState
    extends ConsumerState<_ConnectAntigravityDialog> {
  static const _pendingTimeout = Duration(minutes: 20);

  _OAuthDialogPhase _phase = _OAuthDialogPhase.initializing;
  AntigravityOAuthInit? _init;
  ...
  Future<void> _startOAuth() async {
    ...
    try {
      final init = await repo.initAntigravityOAuth();
  ...
  void _subscribeToWs() {
    ...
    _wsSub = ws.events.listen((ev) {
      ...
      if (integration.provider != 'antigravity_oauth') {
        return;
      }
  ...
  String _reasonText(BuildContext context, String? reason) {
    final l10n = requireAppLocalizations(
      context,
      where: '_ConnectAntigravityDialog._reason',
    );
    switch (reason) {
      case 'user_cancelled':
      case 'access_denied':
  ...
  Widget build(BuildContext context) {
    final l10n = requireAppLocalizations(
      context,
      where: '_ConnectAntigravityDialog',
    );
    return AlertDialog(
      title: Text(l10n.integrationsLlmAntigravityOAuthTitle),
    ...
    switch (_phase) {
      ...
      case _OAuthDialogPhase.awaiting:
        ...
        return Column(
          ...
          children: [
            Text(l10n.integrationsLlmAntigravityOAuthStep1),
            const SizedBox(height: 16),
            Row(
              children: [
                Text(
                  l10n.integrationsLlmAntigravityOAuthCode,
                ...
                IconButton(
                  tooltip: l10n.integrationsLlmAntigravityOAuthCopy,
            ...
            Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    l10n.integrationsLlmAntigravityOAuthWaiting,
      ...
      case _OAuthDialogPhase.timeout:
        return Padding(
          padding: const EdgeInsets.symmetric(vertical: 16),
          child: Text(l10n.integrationsLlmAntigravityOAuthTimeout),
        );
    }
  }
      ...
      case _OAuthDialogPhase.awaiting:
        return [
          ...
          FilledButton.icon(
            ...
            label: Text(l10n.integrationsLlmAntigravityOpenBrowser),
          ),
        ];
      case _OAuthDialogPhase.success:
        return [
