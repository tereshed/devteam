...
class UserSecretRepository {
  UserSecretRepository({required Dio dio}) : _dio = dio;
  final Dio _dio;
  ...
  Future<List<SecretRefModel>> list({CancelToken? cancelToken}) async {
    try {
  ...
  Future<SecretRefModel> set({required String keyName, required String value, CancelToken? cancelToken}) async {
    try {
  ...
  Future<void> delete(String secretId, {CancelToken? cancelToken}) async {
    try {
      await _dio.delete(
        '/me/secrets/$secretId',
        cancelToken: cancelToken,
